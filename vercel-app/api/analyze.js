...
async function normalizeScreenshot(input) {
  ...
  return {
    buffer: vision.data,
    width: vision.info.width,
    height: vision.info.height,
    ocrBuffer: ocr.data,
    ocrWidth: ocr.info.width,
    ocrHeight: ocr.info.height,
    mime: 'image/jpeg',
    hash: crypto.createHash('sha256').update(vision.data).digest('hex'),
  };
}
...
async function makeAxisVariant(shot, leftFraction, threshold = null, scale = 1.2) {
  ...
  let pipeline = sharp(sourceBuffer)
    ...
    .grayscale()
    .normalize()
    .sharpen({ sigma: 1.05, m1: 1.0, m2: 2.0 });
...
async function getWorker() {
  if (!workerPromise) {
    workerPromise = createWorker(ENG_DATA.code || 'eng', 1, {
...
async function recognizeAxis(variant, psm) {
  ...
  try {
    ...
    return await timeoutPromise(
      worker.recognize(variant.buffer, {}, { hocr: true }),
      OCR_PASS_TIMEOUT_MS,
      'Local price-axis OCR pass timed out.',
    );
  } catch (error) {
...
function attemptSummary(attempts) {
  return attempts.map(x => ({
    labels: x.samples.length,
    elapsedMs: x.elapsedMs,
    error: x.error,
    psm: x.options.psm,
    cropRightPercent: Math.round((1 - x.options.leftFraction) * 100),
  }));
}
...
async function calibrateScreenshot(shot) {
  ...
  if (spent < 7_500 && !attempts.some(x => /timed out/i.test(x.error))) {
    ...
    attempts.push(tertiary);
    samples = [...samples, ...tertiary.samples];
    calibration = calibratePriceAxis(samples);
  }
...
function validateNativePlan(native, shots, calibrations) {
  ...
  const entry = priceForPermille(calibration, best.entryY);
  const stop = priceForPermille(calibration, best.stopY);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (decoded.reduce((n, s) => n + s.buffer.length, 0) > MAX_TOTAL_BYTES) {
    ...
    const seen = new Set();
    for (const shot of shots) {
      if (seen.has(shot.hash)) return json(res, 400, { ok: false, requestId, error: 'Duplicate screenshots detected. Each slot must contain a distinct chart.' }, requestId);
      seen.add(shot.hash);
    }

    const fingerprint = requestFingerprint(shots);
    ...
    const message = /timed out/i.test(error?.message || '')
      ? 'Local analysis timed out safely. No trade was produced.'
      : (error?.message || 'Native analysis failed safely. Please retry.');
